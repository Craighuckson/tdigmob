...
def write_parse_file(form,ticket_number,page_number,total_pages,units):
    ...
    with open(f'{ticket_number}-{page_number}.txt',mode='a') as tf:
        ...
        tf.write(units)
    p = os.getcwd()
    sg.popup_get_text(f'File written to {p}\{ticket_number}-{page_number}.txt')
...
def main():
    ...
    try:
        ticket_number = int(sg.popup_get_text('Enter ticket number','Ticket Number'))
        totalpages = int(sg.popup_get_text('Enter number of pages','Pages'))
    except ValueError:
        sg.popup('Must enter a valid number')
        return
